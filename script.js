document.addEventListener('DOMContentLoaded', function () {
  ...
  document.body.addEventListener('submit', function (e) {
    ...
    fields.forEach(function (field) {
      ...
      if (isRequired && value === '') {
        showFieldError(field, getErrorMessage('required', field));
        isValid = false;
      ...
      if (fieldType === 'email') {
        ...
        if (!emailRegex.test(value)) {
          showFieldError(field, getErrorMessage('email', field));
          isValid = false;
          if (!firstInvalidField) firstInvalidField = field;
          return;
        }
      ...
      if (fieldType === 'tel') {
        ...
        if (!telRegex.test(value)) {
          showFieldError(field, getErrorMessage('tel', field));
          isValid = false;
          if (!firstInvalidField) firstInvalidField = field;
          return;
        }
      ...
      if (field.minLength && field.minLength > 0 && value.length < field.minLength) {
        showFieldError(field, getErrorMessage('minlength', field));
        isValid = false;
        if (!firstInvalidField) firstInvalidField = field;
        return;
      }
      ...
      if (field.maxLength && field.maxLength > 0 && value.length > field.maxLength) {
        showFieldError(field, getErrorMessage('maxlength', field));
        isValid = false;
        if (!firstInvalidField) firstInvalidField = field;
        return;
      }
      ...
      if (patternAttr) {
        const patternRegex = new RegExp('^(?:' + patternAttr + ')$');
        if (!patternRegex.test(value)) {
          showFieldError(field, getErrorMessage('pattern', field));
          isValid = false;
          if (!firstInvalidField) firstInvalidField = field;
        }
      }
  ...
  function getErrorMessage(type, field) {
    const label = getFieldLabel(field);
    const messages = {
      required: label + ' is required.',
      email: 'Please enter a valid email address.',
      tel: 'Please enter a valid phone number.',
      minlength: label + ' must be at least ' + field.minLength + ' characters.',
      maxlength: label + ' must be no more than ' + field.maxLength + ' characters.',
  ...
  function getFieldLabel(field) {
    ...
    if (id) {
      ...
      if (labelEl) return labelEl.textContent.trim().replace(/:$/, '');
    }
  ...
  (function applyRTL() {
    ...
    const isRTL = htmlEl.getAttribute('dir') === 'rtl' ||
                  htmlEl.getAttribute('lang') === 'ar' ||
                  htmlEl.getAttribute('lang') === 'he' ||
                  htmlEl.getAttribute('lang') === 'fa' ||
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "TERRA משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '201e6338-0f57-49e0-957a-031d1a87e0cb',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '201e6338-0f57-49e0-957a-031d1a87e0cb';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '201e6338-0f57-49e0-957a-031d1a87e0cb';
